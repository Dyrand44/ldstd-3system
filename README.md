...
Let hierbij goed op, er worden Yes/No vragen gesteld en deze kun je beantwoorden met 'y' of 'n' De vragen kun je als volgt beantwoorden:
...
Nu is de database server klaar om in gebruik genomen te worden, hier moet echter nog een database en user voor aangemaakt worden. Eerst log je in op de MariaDB server dmv je de root user met bijbehorend wachtwoord
...
Als laatste stap moet nginx nog weten hoe hij php moet aansturen en dat regelen wij als volgt in: Ga naar de map waar de config moet staan en haal de file op met wget. Herstart daarna nginx zodat de config ingeladen wordt.
...
Dan zijn er nog twee files die hand in hand gaan, de ops.json en de whitelist.json. ops voor de server admins en de whitelist zodat vreemden niet inloggen.
